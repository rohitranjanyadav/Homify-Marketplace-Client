  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter(
      (product) =>
        ...
        product.id.toLowerCase().includes(normalizedSearchTerm),
    );
  }, [products, searchTerm]);

  const deleteProduct = (id: string) => {
    if (id) dispatch(handleProductItemDelete(id));
  };
            ...
            <tbody>
              {filteredProducts.length > 0 ? (
                ...
                <tr>
                  <td
                    colSpan={5}
                    className="px-5 py-12 text-center text-sm text-[#7a685d]"
                  >
                    {searchTerm
                    ...
                    {searchTerm && (
                      <button
                        ...
                        className="ml-3 font-medium text-[#a65f3b] underline underline-offset-4"
                      >
                        Clear search
                      </button>
                    )}
